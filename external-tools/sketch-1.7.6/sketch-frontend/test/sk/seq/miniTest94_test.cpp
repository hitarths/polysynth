...
void reverseSketch_ANONYMOUSTest(Parameters& _p_) {
  for(int _test_=0;_test_< _p_.niters ;_test_++) {
    ...
    try{
      ANONYMOUS::reverse(in1,in2,_out_outsk);
      ANONYMOUS::reverseSketch(in1,in2,_out_outsp);
    }catch(AssumptionFailedException& afe){  }
    for(int _i_=0;_i_<5;_i_++) {
      if(_out_outsk[_i_]!=_out_outsp[_i_]) {
        printf("Automated testing failed in reverseSketch_ANONYMOUSTest\n");
        cout<<"in1 = "<<printArr(in1, 5)<<endl;
        cout<<"in2 = "<<printArr(in2, 5)<<endl;
...
int main(int argc, char** argv) {
  ...
  printf("Automated testing passed for miniTest94\n");
  return 0;
}
