...
void mergeSort_ANONYMOUSTest(Parameters& _p_) {
  for(int _test_=0;_test_< _p_.niters ;_test_++) {
    if(3==0){ continue; }
    bool*  input_0= new bool [3];
    for(int _i_=0;_i_<3;_i_++) {
      input_0[_i_]=((rand()%2) == 1);
    }
    if(_p_.verbosity > 2){
      cout<<"input_0=[";
      for(int _i_=0;_i_<3;_i_++) {
    ...
    try{
      ANONYMOUS::sort(input_0,_out_outsk);
      ANONYMOUS::mergeSort(input_0,_out_outsp);
    }catch(AssumptionFailedException& afe){  }
    for(int _i_=0;_i_<3;_i_++) {
      if(_out_outsk[_i_]!=_out_outsp[_i_]) {
        printf("Automated testing failed in mergeSort_ANONYMOUSTest\n");
        cout<<"input_0 = "<<printArr(input_0, 3)<<endl;
...
int main(int argc, char** argv) {
  ...
  printf("Automated testing passed for miniTest42\n");
  return 0;
}
