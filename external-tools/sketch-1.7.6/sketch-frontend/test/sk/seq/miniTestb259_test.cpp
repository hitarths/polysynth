...
void testSK5_ANONYMOUSTest(Parameters& _p_) {
  for(int _test_=0;_test_< _p_.niters ;_test_++) {
    ...
    try{
      ANONYMOUS::test5(N,in,_out_outsk);
      ANONYMOUS::testSK5(N,in,_out_outsp);
    }catch(AssumptionFailedException& afe){  }
    for(int _i_=0;_i_<N;_i_++) {
      if(_out_outsk[_i_]!=_out_outsp[_i_]) {
        printf("Automated testing failed in testSK5_ANONYMOUSTest\n");
        cout<<"N = "<<N<<endl;
...
int main(int argc, char** argv) {
  ...
  srand(time(0));
  testSK5_ANONYMOUSTest(p);
  printf("Automated testing passed for miniTestb259\n");
  return 0;
}
