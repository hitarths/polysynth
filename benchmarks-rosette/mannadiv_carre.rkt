...
(define (call_loop lst )#| lst = x r a |#
 ...
 #| (list (* p 0.5) (syn_fun2 p q) (syn_fun1 p q r) a)|#
  ...
  (if (equal? t x) (list (+ x 1) (syn_fun1 x y) 0 n) (list x (- y 2) (+ t 1) n))
)
...
(define (arch x y t n)
  ...
  (assert (implies (and (>= (* y (- y 1)) 0) (loop_inv (list x y t n))) (loop_inv (call_loop (list x y t n))) )) 
  (assert (implies (and (< (* y (- y 1)) 0) (loop_inv (list x y t n))) (post_cond (list x y t n))))
)
